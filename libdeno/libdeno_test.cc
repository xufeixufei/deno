...
TEST(LibDenoTest, InitializesCorrectly) {
  ...
  Deno* d = deno_new(snapshot, empty, nullptr, nullptr);
  EXPECT_TRUE(deno_execute(d, nullptr, "a.js", "1 + 2"));
  deno_delete(d);
}

TEST(LibDenoTest, InitializesCorrectlyWithoutSnapshot) {
  Deno* d = deno_new(empty, empty, nullptr, nullptr);
  EXPECT_TRUE(deno_execute(d, nullptr, "a.js", "1 + 2"));
  deno_delete(d);
}

TEST(LibDenoTest, Snapshotter) {
  const char* a_src = SET_GLOBAL "global.a = 1 + 2";
  Deno* d1 =
      deno_new_snapshotter(empty, nullptr, nullptr, "a.js", a_src, nullptr);
  deno_buf test_snapshot = deno_get_snapshot(d1);
  // TODO(ry) deno_delete(d1);

  Deno* d2 = deno_new(test_snapshot, empty, nullptr, nullptr);
  const char* b_src = SET_GLOBAL "if (a != 3) throw Error('x');";
  EXPECT_TRUE(deno_execute(d2, nullptr, "b.js", b_src));
  deno_delete(d2);
}
...
TEST(LibDenoTest, RecvReturnEmpty) {
  ...
  Deno* d = deno_new(snapshot, empty, recv_cb, nullptr);
...
TEST(LibDenoTest, Shared) {
  ...
  Deno* d = deno_new(snapshot, shared, nullptr, nullptr);
...
TEST(LibDenoTest, ModuleResolution) {
  ...
  auto resolve_cb = [](auto _, const char* specifier, const char* referrer) {
    EXPECT_STREQ(specifier, "b.js");
    EXPECT_STREQ(referrer, "a.js");
    count++;
    const char* src_b = "export function retb() { return 'b'; }";
    return deno_module{"b.js", src_b};
  };
  Deno* d = deno_new(empty, empty, nullptr, resolve_cb);
  EXPECT_TRUE(deno_execute(d, nullptr, "a.js", src_a));
  EXPECT_EQ(count, 1);
  deno_delete(d);
}
